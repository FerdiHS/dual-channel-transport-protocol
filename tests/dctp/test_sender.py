...
def test_ack_marks_acked_and_slides_window():
    ...
    a = Packet(typ=PacketType.ACK, channel_type=0, seq=0, ts_send=0, ack=100, rcv_wnd=0, ts_echo=0, payload=b"")
    s.on_feedback(a)
    
    accepted = s.offer(b"B"*100)
    assert accepted == 100

def test_sack_marks_higher_ranges():
    """Test that receiving a SACK marks the specified segments as acknowledged."""
    s = Sender(mss=100, window=400, now_ms=fake_clock)
    s.offer(b"A"*400)
    _ = s.due_packets()
    
    sa = Packet(typ=PacketType.SACK, channel_type=0, seq=0, ts_send=0, ack=0, rcv_wnd=0, ts_echo=0,
                sack=[(200,300),(300,400)], payload=b"")
